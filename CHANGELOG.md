...
### Fixed
- `CatalogWarning` and `CatalogException` survive `pickle` and `copy.deepcopy` unchanged. Python rebuilds an exception as `type(e)(*e.args)`, which assumes the first constructor argument is the message; these classes render the message from structured data and their subclasses name that data first, so the rendered sentence was fed back into a domain field and the template rendered around its own output. `UnknownAtomNameWarning(atom_name="Ar")` came back from `pickle` reading `Atom name 'Atom name 'Ar' is not recognized…'`. Both now rebuild from state, keeping `code`, `extra` and the exact text, and it works for a subclass that computes its message in `__init__` — which no reconstruction through the constructor can.

  One behaviour changes with it: an `extra` holding something unserializable now raises on `pickle` instead of succeeding and delivering corrupted text on the far side. `extra` already had to be serializable to reach JSON events and exported bundles.

  This does not touch pytest-xdist, which calls `cls(*args)` itself rather than pickling. That half is proposed upstream as `pytest-dev/pytest-xdist#1372`.
